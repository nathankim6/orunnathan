...
선생님마다 `persona.md`(1인칭 출제 규칙 — "나는 윤은영이다")가 있다. **예측과 출제는 그 파일을
읽고 그 선생님이 되어서 한다** — 지문을 그 선생님의 눈으로 읽고, 지문 특성에 맞으면 아직 안 낸
유형도 `[신규 유형]` 으로 예측한다. 절차는 스킬 §4~§6.

지금 쌓인 것: 흑석고 1학년 공통영어1 두 분(윤은영 영어B · 전정이 영어A), 2026 1학기 중간·기말
두 시험(각 28문항 공동 출제) · 교과서 2과 워크시트 1벌 · 기말 직전보충 자료, 프로파일 v3 · 페르소나.
2026 2학기 중간: 범위 원문 · 지문 분석 · 적중 모의고사 2회(`schools/heukseok-go/shared/2026-2-중간/`)와
선생님별 예측 v1 — 예측이라 페르소나·프로파일은 그대로다. 시험이 나오면 스킬 §6 으로 채점해 v4 로 올린다.

...
## 선생님 기억 (`teacher-memory/`) — 웹앱 없이 대화로 쌓는 쪽

ORUN ORACLE 과 같은 일(문항 데이터화 → 출제 성향 프로파일 → 예측 → 적중 모의고사)을 **웹앱 없이
이 저장소 파일과 대화만으로** 한다. 사용자가 "선생님 기출 여기 있어요" 처럼 말하면 스킬
`teacher-memory`(`.claude/skills/teacher-memory/SKILL.md`)를 따라 이 폴더에 쌓는다. 배포되지 않는다.

**이 폴더가 곧 기억이다.** Claude 는 대화가 끝나면 다 잊는다 — 다음 대화가 이어받는 것은 오로지
여기 커밋된 파일뿐이다. 그래서 지킬 것 셋 —

- **자료를 받으면 그 자리에서 커밋한다.** 커밋하지 않은 분석은 대화가 끝나는 순간 사라진다.
- **`main` 에 머지해 둔다.** 새 세션은 기본 브랜치를 받아 시작하므로, 작업 브랜치에만 있으면
  다음 대화의 Claude 에게는 그 기억이 없는 것과 같다.
- **`profile.md` 는 덮어쓰지 않는다.** v1 → v2 → v3 로 쌓고 옛 판은 `<details>` 로 접어 남긴다.
  "성향이 어떻게 바뀌어 왔는가"(드리프트)가 예측의 핵심 재료다.

지금 쌓인 것: 흑석고 1학년 공통영어1 두 분(윤은영 영어B · 전정이 영어A), 2026 1학기 중간·기말
두 시험(각 28문항 공동 출제), 프로파일 v2. 시험이 더 쌓이면 예측·적중 모의고사로 넘어간다.
...
- **화면은 반드시 브리지가 내주는 주소(`http://127.0.0.1:<문>/`)에서 연다.** 이것이
  구독 방식이 안 되던 진짜 까닭이었다 — 크롬은 공개 https 사이트가 이 컴퓨터 안을
  부르면 그 요청을 **붙잡아 둔다**(로컬 네트워크 권한). 거절도 아니고 답이 영영 오지
  않아 화면이 "확인하는 중…" 에서 멈춘다. 재현해 보면 이렇다 —

  | 화면을 연 곳 | 127.0.0.1 브리지 부르기 |
  |---|---|
  | `https://nathankim6.github.io` | 영영 안 옴 (5초 뒤 강제 취소) |
  | `http://127.0.0.1` | 54ms · 성공 |

  그래서 브리지(1.2.0)가 `GET /` · `/mock-exam.html` 로 **생성기 화면까지 내준다.**
  사본을 만드는 것이 아니라 배포된 그 한 벌을 받아 와 그대로 흘려 주고
  (`orun-app.html` 에 받아 둔 것은 인터넷이 끊겼을 때 쓰는 여벌일 뿐이다),
  화면과 중계소가 같은 자리가 되어 크롬이 막을 것이 없어진다. CORS 도 필요 없다.
  · 런처는 `--open` 으로 주소만 넘기고 **브라우저는 브리지가 다 뜬 뒤 스스로 연다.**
    런처가 먼저 열면 아직 듣지 않아 빈 화면이 난다.
  · 여는 명령은 컴퓨터마다 다르다 — `openerList()` 가 맥 `open`, 윈도우 `start`,
    리눅스 `xdg-open`·`gio open` 을 **차례로** 해 보고 다 안 되면 못 열었다고 말한다.
    있기는 한데 열지 못하고 끝나는 것도 있어(`gio` 같은) **끝난 값이 0 이 아니면
    다음 것으로** 넘어간다.
  · `spawn` 은 못 찾으면 예외가 아니라 `error` 이벤트로 온다. 안 받으면 브리지가
    통째로 죽는다 — `.on("error", …)` 를 반드시 단다.
  · 화면 쪽 `bridgeUrl()` 은 저장된 것이 없고 지금 주소가 루프백이면 **그 자리**를 쓴다.
  · 기다림에는 끝을 둔다(`abortAfter()`). 공개 주소에서 열려 막히면 그 사실을
    글로 알려 준다 — **앱에 이미 `withTimeout(promise, ms, msg)` 이 있으니 이름을 겹치지 말 것.**
    (한 번 겹쳐서 파일 읽기까지 통째로 깨뜨린 적이 있다.)
...
- 회귀 검사: **`subsim.mjs`(구독 경로 처음부터 끝까지 — 런처 두 번 누르기 → 브리지 →
  브리지가 내준 화면 → 연결 → 올리기 → 분석 → 출제 → 시험지가 나오는가. 가짜 도구는
  `fakebin2/`, 답은 `sample/replies.json` 녹음본을 쓴다)** ·
  `relaunch.sh`(런처 — 두 번 누르기·남의 문·실패 표시·CRLF) ·
  `macopen.mjs`(브라우저 여는 길 — 있는 것을 찾을 때까지·하나도 없을 때) ·

...
(async () => {
  ...
  await page.evaluate(([id, name, text]) => { ORACLE.UI.openFilesSheet([new File([text], name, { type: "text/plain" })], id); }, [tid, "2025_1학기_중간_대비_프린트.txt", fx("handout.txt")]);
  await page.waitForSelector("#fsMemo");
  ok(await page.evaluate(() => document.getElementById("sheetWrap").classList.contains("on") && document.querySelector("#sheetBody [data-k]").textContent === "프린트"), "파일 시트가 열리고 이름으로 프린트를 알아봤다");
  await page.fill("#fsMemo", "이 프린트는 어법 정리 위주예요. 1학기 중간 대비용.");
  await page.screenshot({ path: path.join(T, "out") + "/e2e-0-files-sheet.png" });
  ...
  const memoSrc = await page.evaluate(async (id) => (await ORACLE.DB.where("sources", "teacherId", id)).find(x => x.kind === "프린트").memo, tid);
  ok(memoSrc === "이 프린트는 어법 정리 위주예요. 1학기 중간 대비용." && calls.some(c => /프린트\(학습지\)/.test(c.kind) && c.memo === memoSrc) && !calls.some(c => /시험범위 자료/.test(c.kind) && c.memo), "메모가 프린트 자료에 저장되고 프린트 색인 프롬프트에만 실렸다");
  ok(await page.evaluate(() => document.getElementById("intelBody").textContent.includes("✎ 이 프린트는")), "큐 줄에 메모 표시");
  await enq("2025_1학기_중간_기출.txt", fx("exam1.txt"));
  await page.evaluate(([id, name, text]) => { ORACLE.APP.enqueue([new File([text], name, { type: "text/plain" })], id, null, { memo: "2학기 기말 시험지예요. 서술형은 마지막 장에 있어요", kinds: ["exam"] }); }, [tid, "기출_아무이름.txt", fx("exam2.txt")]);
  ...
  ok(memoExam && memoExam.file === "기출_아무이름.txt" && /2학기 기말/.test(memoExam.label) && calls.filter(c => /기출 시험지에서 추출한/.test(c.kind) && c.memo === memoExam.memo).length >= 1, "메모를 붙인 기출: 종류 강제 · 메모의 '2학기 기말' 이 시험 정보에 잡힘 · 데이터화 프롬프트에 실림 " + JSON.stringify(memoExam));

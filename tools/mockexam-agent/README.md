...
A4 세로 · 여백 좌우 20 / 위 18 / 아래 15 mm · 제목(16pt 굵게, 가운데) · 부제 · 반/번호/이름 줄(밑줄) ·
본문 2단(간격 8 mm) · 함초롬바탕 10pt · 지문·대화는 0.12 mm 검정 실선 상자(1×1 표) · 선지 ①~⑤ ·
서술형은 `<조건>` 상자와 `→ ____` 답란 · 맨 뒤 새 쪽에 1단 정답표(번호·정답·배점·출처·해설).
다른 학교 판형이 필요하면 `PAGE` · `COL_GAP_MM` · `FONT` · `BODY_PT` 와 `build()` 를 고친다.
